...
<body>
	...
	<?php
        ...
        $registro = mysql_query("SELECT * FROM productos, tipoproductos, clientes, ventas, detalleventa WHERE fecha_venta = '$fecha' AND id_venta = id_venta_detalle AND id_prod = id_prod_detalle AND id_clien_venta = id_clien AND estadodetalle=0 AND tipo_prod = id_tipo_pro"); 
        if(!empty($registro)){
    ...
    <script>
    function eliminarProducto(iddetalle, idventa){
		var url = 'eliminaVentasEntreFechas.php';
		var pregunta = confirm('¿Esta seguro de eliminar esta Venta?');
		if(pregunta==true){
			$.ajax({
			type:'POST',
			url:url,
